...
import ex.ample.httpstapir.api._
import ex.ample.httpstapir.config._
import ex.ample.httpstapir.db.FlywayDatabaseMigrator
...
object Server extends IOApp.WithContext {
  ...
  override def run(args: List[String]): IO[ExitCode] = {
    ...
    val program = for {
      ...
      docs             = OpenAPIDocsInterpreter().toOpenAPI(List(HelloWorld.greetings), "https-tapir", "1.0.0")
